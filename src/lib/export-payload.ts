...
export async function buildExportPayload(eventId: string, type: ExportType, includeSensitiveFields: boolean): Promise<Array<Record<string, unknown>>> {
  ...
  if (type === "NOMINATIONS_CLEAN") {
    return db.select({ Category: categories.name, Nominee: nominees.name, Biography: nominees.bio, Status: nominees.status, Source: nominees.source, NominationCount: authoritativeNominationCount }).from(nominees).innerJoin(categories, eq(nominees.categoryId, categories.id)).where(eq(nominees.eventId, eventId)).orderBy(categories.displayOrder, nominees.displayOrder);
  }
  if (type === "VOTES_RAW") {
    const rows = await db.select({ BallotId: voteSessions.id, Category: categories.name, Nominee: nominees.name, Skipped: votes.skipped, VerificationMethod: voteSessions.verificationMethod, VerifiedEmail: voteSessions.verifiedEmail, InvitationCode: sql<string>`'[redacted]'`, DeviceFingerprint: voteSessions.deviceFingerprint, IPAddress: voteSessions.ipAddress, Status: voteSessions.status, SubmittedAt: voteSessions.submittedAt }).from(votes).innerJoin(voteSessions, eq(votes.voteSessionId, voteSessions.id)).innerJoin(categories, eq(votes.categoryId, categories.id)).leftJoin(nominees, eq(votes.nomineeId, nominees.id)).where(and(eq(votes.eventId, eventId), eq(voteSessions.status, "SUBMITTED"))).orderBy(desc(voteSessions.submittedAt));
  ...
  if (type === "ANALYTICS") {
    const [nominationCount, nomineeCount, accounting] = await Promise.all([
      ...
      getEventVoteAccounting(eventId),
    ]);
    return [{
      RawNominations: nominationCount[0]?.count ?? 0,
      CleanedNominees: nomineeCount[0]?.count ?? 0,
